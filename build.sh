...
pushd lib
if [ -e libavcodec.so    ]; then rm -f libavcodec.so   ; ln -s libavcodec.so.60.10.100    libavcodec.so   ; fi
if [ -e libavdevice.so   ]; then rm -f libavdevice.so  ; ln -s libavdevice.so.60.2.100    libavdevice.so  ; fi
if [ -e libavfilter.so   ]; then rm -f libavfilter.so  ; ln -s libavfilter.so.9.6.100     libavfilter.so  ; fi
if [ -e libavformat.so   ]; then rm -f libavformat.so  ; ln -s libavformat.so.60.5.100    libavformat.so  ; fi
if [ -e libavutil.so     ]; then rm -f libavutil.so    ; ln -s libavutil.so.58.6.100      libavutil.so    ; fi
if [ -e libswresample.so ]; then rm -f libswresample.so; ln -s libswresample.so.4.11.100  libswresample.so; fi
if [ -e libswscale.so    ]; then rm -f libswscale.so   ; ln -s libswscale.so.7.2.100      libswscale.so   ; fi
